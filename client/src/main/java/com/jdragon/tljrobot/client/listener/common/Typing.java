...
//                System.out.println("退格+2");
    ...
    void changePosition() {// 自动滚动条翻页方法
        ...
        int temp;
        widthFontNum = (TypingAndWatching().getWidth() - fontWidth) / fontSize;
        cursor = cursor % (LocalConfig.typePageCount + (widthFontNum + 1) / 3);
        if (thisPageNum == 0) {
            while (thisPageTypeStr.length() > cursor) {
                if (heightFontNum > 2)
                    temp = (heightFontNum - 1) * (widthFontNum + 1);
                else
                    temp = widthFontNum + 1;
            ...
            while (thisPageTypeStr.length() + (widthFontNum + 1) / 3 > cursor) {
                if (heightFontNum > 2)
                    temp = (heightFontNum - 1) * (widthFontNum + 1);
                else
                    temp = widthFontNum + 1;
        ...
        if (thisPageTypeStr.length() == 1) {
            if (heightFontNum > 1) {
                cursor = (heightFontNum - 1) * (widthFontNum + 1) + (widthFontNum + 1) / 3;
            } else {
                cursor = (widthFontNum + 1) - (widthFontNum + 1) / 3;
            }
